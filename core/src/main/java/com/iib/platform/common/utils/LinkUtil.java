...
/**
 * Link Util - Static utility helpers for link functions
 * 
 * @author TechChefz
 */
...
public final class LinkUtil {
	...
	/**
	 * Method is going to check whether passed url is AEM internal or external, if
	 * it is internal URL returned url will be suffixed by .html else will be
	 * returned as it is.
	 * 
	 * @param internalUrl
	 * @return
	...
	/**
	 * Convenience wrapper around createExternalizedLinkFromAuthoredProperty(String,
	 * String, ResourceResolver, HttpServletRequest) passing in / as the
	 * internalLinkPrefix.
	 * 
	 ...
	 * @param resourceResolver <code>ResourceResolver</code> current request's
	 *                         resource resolved
	 ...
	 * <ul>
	 * <li>Will map a link if required in order to ensure any JCR resource resolver
	 * mappings are applied to the link. For links being printed out in HTML this
	 * method is not required - the link checker will apply the mappings. This
	 * method is intended to used in situations where the link checker will not be
	 * run, for example when doing a 301 redirect server side, or printing out links
	 * in JS files or XML files.</li>
	 * <li>Ensures that link valid for use in href or src attributes by adding an
	 * extension and network protocol if required.</li>
	 * </ul>
	 * 
	 * @see LinkUtils.mapLink(String, String, ResourceResolver, HttpServletRequest)
	 * @see LinkUtil.createValidLinkFromAuthoredProperty(String, String)
	 * 
	 * @param link               <code>String</code> to be evaluated
	 * @param internalLinkPrefix <code>String</code> internal link prefix used to
	 *                           determine if the link is an internal link
	 * @param resourceResolver   <code>ResourceResolver</code> current request's
	 *                           resource resolved
	 * @param request            <code>HttpServletRequest</code> current HTTP
	 *                           request - optional but less capable resource
	 *                           mapping.
	 * @return String
	 */
	 ...
	 * attributes but adding and extension if required and adding network protocol
	 * if required.
	 * 
	 ...
	 * 
	 * @param link               <code>String</code> link to be checked.
	 * @param internalLinkPrefix <code>String</code> pattern that indicates if a
	 *                           link is internal or not.
	 * @return String
	 */
	 ...
	 * method is intended to used in situations where the link checker will not be
	 * run, for example when doing a 301 redirect server side, or printing out links
	 * in JS files or XML files.
	 * 
	 * This method runs LinkUtils.isInternalLink(String) to determine if the link is
	 ...
	 * @param link               <code>String</code> to be evaluated
	 * @param internalLinkPrefix <code>String</code> internal link prefix used to
	 *                           determine if the link requires mapping.
	 * @param resourceResolver   <code>ResourceResolver</code> current request's
	 *                           resource resolved
	 * @param request            <code>HttpServletRequest</code> current HTTP
	 *                           request - optional but less capable resource
	...
	/**
	 * Determines if a link is an internal link based on the internal link prefix
	 * passed in. Defaults to / as the internal link prefix if one is not provided.
	 * Examples
	 * <ul>
	 * <li>link = /content/site/landing and link internalLinkPrefix = / result is
	 * true</li>
	 * <li>link = http://mysite.com/products and link internalLinkPrefix = / result
	 * is false</li>
	...
	/**
	 * Checks to see if a link requires and extension and if required adds .html.
	 * Following items are checked to determine if a link requires an extension:
	 * <ul>
	...
	private static boolean linkNeedsExtension(String link, String internalLinkPrefix) {
		return EXTENSIONS_REQUIRED && isInternalLink(link, internalLinkPrefix)
				&& !StringUtils.containsAny(link, new char[] { '.', '?' });
	}
	 ...
	 * </ul>
	 * 
	 * This method does not support relative links - it assumes that a relative link
	 * is in fact link with server name that needs a protocol.
	 * 

...
/*function landlineCheck(){
    $('#LandlineNumber').on("blur change", function(){
        if($(this).val() != '') {
            if($('#STDNumber').val() == '') {
				$('#STDNumber').attr('data-validation','required,numbersOnly,minlength');
            }
            if($('#ISDNumberLandline').val() == '') {
            	$('#ISDNumberLandline').attr('data-validation','required,numbersOnly');
            }
        }
        else {
			$('#STDNumber').attr('data-validation','numbersOnly,minlength');
            $('#ISDNumberLandline').attr('data-validation','numbersOnly');

            if($('#STDNumber').val() == '' && $('#LandlineNumber').val()=='') {
				$('#STDNumber').parents('.fieldBox').removeClass('error-field');
             	 $('#STDNumber').parents('.fieldBox').addClass('isValid');
                $('#STDNumber').prev('.placeholder').removeClass('active');
            }

			if($('#ISDNumberLandline').val() == '') {
				$('#ISDNumberLandline').parents('.fieldBox').removeClass('error-field');
             	 $('#ISDNumberLandline').parents('.fieldBox').addClass('isValid');
                $('#ISDNumberLandline').prev('.placeholder').removeClass('active');
            }
        }
...
var landlinetboxflag;
function landlineCheck(){
    $('#LandlineNumber').on("blur change", function(){
        if($(this).val() != '') {
			landlinetboxflag=true;
        }
        else {
            landlinetboxflag=false;
        }
    });
...
function validateMe(_this){
    ...
    $(_this).closest(".fieldBox").removeClass("isValid");
        ...
        var validEmail = /^([a-zA-Z0-9_\-\.]+)@([a-zA-Z0-9_\-\.]+)\.([a-zA-Z]{2,3})$/;
    	...
    	var validMobile = /^[1][0-9].*$/
        var validPan = /^([a-zA-Z]{5})(\d{4})([a-zA-Z]{1})$/
        var validAccount = /^[1][0-9].*$/
        var checkzero = /^0+$/i
        ...
        for(var i=0; i < validationType.length; i++){
            if(validationType[i] =="required" &&  (inputVal=="" || inputVal== false || inputVal== "-1" || inputVal=="+" )){ // for normal select dropdown 
            ...
            if(inputVal != "" && validationType[i].indexOf("minlength") != -1){
                ...
                var minLen = parseInt($(_this).attr("minlength"));
                var typeOfField = validationType.indexOf("numbersOnly") != -1 ? "digits" : "letters";

                if(inputVal.length < minLen){
                    $(_this).closest(".fieldBox").addClass("error-field");
                    //$(_this).siblings(".error").text($(_this).attr("name")+" should not be lesser than "+splitMe[1]+" characters");
                     $(_this).siblings(".error").text(thisField+" should not be less then "+minLen+" "+typeOfField);
             ...
             if(inputVal != "" && validationType[i] == "alphabetOnly" && validAlpha.test(inputVal) == false){
            ...
            if(inputVal != "" && validationType[i] == "accountVal" && validAccount.test(inputVal) == false){
            ...
            if(inputVal != "" && validationType[i] == "zeroCheck" && checkzero.test(inputVal) == true){
                $(_this).closest(".fieldBox").addClass("error-field");
                $(_this).siblings(".error").text("Please enter valid "+thisField);
            ...
            if(inputVal != "" && validationType[i] == "checkData"){
                if(landlinetboxflag==true) {
					if($('#STDNumber').val() == '' || $('#STDNumber').val() == '+') {
						$('#STDNumber').parents('.fieldBox').addClass('error-field');
                    	$('#STDNumber').parents('.fieldBox').removeClass('isValid');
                    	$('#STDNumber').prev('.placeholder').addClass('active');
                        invalidCounter++; 
                    }
                    if($('#ISDNumberLandline').val() == '' || $('#ISDNumberLandline').val() == '+') {
                        $('#ISDNumberLandline').parents('.fieldBox').addClass('error-field');
                    ...
                    landlinetboxflag==false;
					$('#LandlineNumber').parents('.fieldBox').addClass('isValid');
                }
            }
...
function submitForm(formId){
    if(formId == "nrLeadReferForm"){
            nrLeadReferFormData();
    }
    if(formId == "nrLeadForm"){
            nrLeadFormData();
    }
}